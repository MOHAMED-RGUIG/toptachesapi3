...
const router = express.Router();
  ...
  router.post('/downloadOrder', async (req, res) => {
    const {
      clientInfo,
      contacts,
      contacts1,
    ...
    try {
        ...
        const pdfFilename = `Fiche-creation-compte-${clientInfo.companyName}.pdf`;
    
        // Send PDF file as a downloadable response
        const pdfBuffer = Buffer.from(pdfDoc.output('arraybuffer'));
        res.setHeader('Content-Type', 'application/pdf');
        res.setHeader('Content-Disposition', `attachment; filename="${pdfFilename}"`);
        res.send(pdfBuffer);  // Send the PDF as a response to be downloaded
      } catch (error) {
